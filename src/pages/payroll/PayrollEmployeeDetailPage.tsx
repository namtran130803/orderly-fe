...
export const PayrollEmployeeDetailPage: React.FC = () => {
  ...
  const month =
    state.month ??
    (monthFromQuery >= 1 && monthFromQuery <= 12
      ? monthFromQuery
      : now.getMonth() + 1);
  const year =
    state.year ??
    (yearFromQuery >= 2000 && yearFromQuery <= 2100
      ? yearFromQuery
      : now.getFullYear());
        ...
        {data && (
          <>
            <div className="mt-4 bg-(--color-bg-surface) border-y border-(--color-border-main) px-4 py-5 text-center">
              ...
              <p className="text-2xl font-semibold text-(--color-success) tabular-nums mt-1">
                ...
                >
                  ...
                  <span
                    className={cn(
                      "text-sm text-right tabular-nums",
                      line.highlight
                        ? "font-semibold text-(--color-success)"
                        : "text-(--color-text-main)",
                    )}
                  >
                    {line.value}
            ...
            <div className="bg-(--color-bg-surface) border-b border-(--color-border-main)">
              ...
              <SummaryRow
                label="Đi làm"
                value={`${data.counts.workDays} ngày`}
              />
              <SummaryRow
                label="Nghỉ có lương"
              ...
              />
              <div className="px-4 py-3 flex items-center justify-between gap-4 border-b border-(--color-border-main) last:border-b-0">
                <span className="flex-none">Nghỉ không lương</span>
                <span className="text-sm text-right tabular-nums text-(--color-warning)">
                  {data.counts.unpaidLeaveDays} ngày
                </span>
              </div>
              <div className="px-4 py-3 flex items-center justify-between gap-4 border-b border-(--color-border-main) last:border-b-0">
                <span className="flex-none">Vắng (không trả)</span>
                <span className="text-sm text-right tabular-nums text-(--color-danger)">
                  {data.counts.absentDays} ngày
                </span>
              </div>
              <div className="px-4 py-3 flex items-center justify-between gap-4 border-b border-(--color-border-main) last:border-b-0">
                <span className="flex-none">Ngày nghỉ theo lịch</span>
                <span className="text-sm text-right tabular-nums text-(--color-text-muted)">
                  {data.counts.offDays} ngày
                </span>
              </div>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p
                        className={cn(
                          "text-xs font-medium",
                          day.status === "WORK" && "text-(--color-success)",
                          day.status === "ABSENT" && "text-(--color-danger)",
                          day.status === "PAID_LEAVE" &&
                            "text-(--color-warning)",
                          day.status === "UNPAID_LEAVE" &&
                            "text-(--color-text-secondary)",
                          day.status === "OFF" && "text-(--color-text-muted)",
                        )}
                      >
                    ...
                    <span
                      className={cn(
                        "text-xs tabular-nums flex-none",
                        day.status === "WORK"
                          ? "text-(--color-success)"
                          : "text-(--color-text-secondary)",
                      )}

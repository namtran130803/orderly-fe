...
export const LeaveDetailPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col relative h-full">
      {p && <LoadingOverlay />}
      <Header title="Chi tiết đơn nghỉ" Icon={Palmtree} backUrl={paths.leave.index} />
        ...
        {r && (
          <>
            <div className="bg-(--color-bg-surface) border-y border-(--color-border-main) px-4 py-3 space-y-1">
              <div className="text-sm font-semibold">{r.employee?.user?.name}</div>
            ...
            {r.status === 'PENDING' && (
              <div className="mt-6 px-4 space-y-3">
                <button
                  type="button"
                  onClick={() => setDlg('ap')}
                  className="w-full py-3 text-sm font-semibold bg-(--color-bg-surface) border-y border-(--color-border-main) text-(--color-success)"
                >
                  Duyệt
                </button>
                <button
                  type="button"
                  onClick={() => setDlg('rj')}
                  className="w-full py-3 text-sm font-semibold bg-(--color-bg-surface) border-y border-(--color-border-main) text-(--color-danger)"
                >
                  Từ chối
                </button>
              </div>
